...
namespace Yhd.TencentSCF.Runtime
{
    ...
    public class SCFHost : ISCFHost, IDisposable
    {
        ...
        protected virtual async Task OnHostStarted()
        {
            ...
            string scf_host = Environment.GetEnvironmentVariable("SCF_RUNTIME_API") ?? "127.0.0.1";
            string scf_port = Environment.GetEnvironmentVariable("SCF_RUNTIME_API_PORT") ?? "9001";
            // _HANDLER -- user defined function entrance, may not be used.
            string handler = Environment.GetEnvironmentVariable("_HANDLER")?? "index.main";
            // user defined value, passed with env. 
            string user_def_env = Environment.GetEnvironmentVariable("myKey") ?? string.Empty;
            // urls.
